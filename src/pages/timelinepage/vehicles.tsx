...
const VehiclesList = () => {
    ...
    if (error) return <div className="text-center mt-4 text-red-600">Error loading vehicles</div>;
    ...
    return (
        <div className="container mx-auto p-4">
            {vehiclesData?.map(({ vehicles, vehicle_specs }) => (
                <div key={vehicles.vehicleId} className="bg-white p-6 rounded-lg shadow-md mb-6">
                    <div className="flex justify-between items-center mb-4">
                        <h3 className="text-2xl font-semibold text-teal-600">{`${vehicle_specs.manufacturer} ${vehicle_specs.model}`}</h3>
                        <Link
                            to={`/bookings?vehicleId=${vehicles.vehicleId}&rentalRate=${vehicles.rentalRate}&vehicleSpecId=${vehicles.vehicleSpecId}`}
                            className="bg-teal-500 text-white px-4 py-2 rounded-md font-bold hover:bg-teal-600 transition duration-300"
                        >
                            Book Now
                        </Link>
                    </div>
